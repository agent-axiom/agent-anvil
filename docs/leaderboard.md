...
GitHub run verification and artifact attestation verification answer different
questions. Run verification proves that the claimed workflow run completed for
the claimed repository and SHA. Artifact attestation verification proves that
GitHub signed a SLSA provenance statement whose subject SHA-256 is the exact
submission file under review. Agent Anvil requires both for the strongest
`github_actions` leaderboard path; neither proves that a visible benchmark
cannot be optimized or gamed.
...
```json
{
  ...
  "generated_by": "agent-anvil/0.2.74"
}
```
...
6. A maintainer can run `anvil leaderboard verify-attestation` to bind the
   downloaded JSON file to GitHub's signed artifact provenance, the claimed
   repository, and the claimed source SHA.
7. A maintainer can run `anvil leaderboard reproduce` to generate a shell script
...
8. The user opens a pull request to the leaderboard submissions repository.
...
11. Maintainers can optionally re-run the agent and mark the row as
...
12. Maintainers can run `anvil leaderboard validate-rerun --github-run` to
...
version.
The artifact attestation verification report uses
`agent-anvil.leaderboard.artifact_attestation_verification.v1`. It records the
enforced repository/source policy, subject SHA-256, verified attestation count,
and a digest of GitHub CLI's JSON result. It does not persist the certificate
bundle or user-controlled predicate payload.

By default, verification denies self-hosted runners and infers
`--source-digest` from `verification.github_sha`. Pin a known workflow with
`--signer-workflow OWNER/REPO/.github/workflows/leaderboard.yml` and optionally
pin `--source-ref refs/heads/main` when the leaderboard policy owns those
constraints. For an offline review, download an attestation bundle separately
and pass `--bundle path/to/bundle.jsonl`.
...
evidence report.
Add `--artifact-attestations` to make `verify-all` call GitHub's cryptographic
verifier for each `github_actions` row and link the compact report from the
evidence index. `audit --require-artifact-attestation` then validates those
reports locally, compares their subject hashes with the actual submission
files, and rejects missing or mismatched proof without making another network
request. The requirement does not replace the separate maintainer-rerun trust
path.
...
The raw `gh` command is useful for interactive inspection. The Agent Anvil
command additionally checks the subject SHA-256, enforces the submission's
claimed repository and source digest, denies self-hosted runners by default,
and emits a stable report that evidence-index audit can consume. Attestations
complement, but do not replace, submission validation, benchmark hashes,
maintainer review, or independent reruns.
