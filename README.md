...
anvil repair runs/latest
anvil compare runs/baseline runs/latest
```

## Demo

The bundled refund-agent demo catches the failure normal answer-only evals miss:
the agent calls `issue_refund` before the order is verified.
...
The offline demo intentionally fails the missing-order refund scenario so the
report has useful failure clusters and repair suggestions.

Use the real OpenAI tool-calling demo agent plus OpenAI semantic grading:
...
External agents can speak a tiny JSONL protocol:
...
```jsonl
{"type":"model_call","model":"my-agent","output_text":"...","tool_calls":[]}
...
- The OpenAI demo agent uses the Responses API with strict function/tool
  schemas and executes returned `function_call` items as local Python tools.
