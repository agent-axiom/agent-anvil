...
continuous integration. In a compact offline benchmark of 30 trials across
refund, tool-safety, and external-agent protocol scenarios, a final-answer
baseline marks 100.0\% of trials as passing, while Agent Anvil's trace-aware
checks mark 33.3\% as passing and expose 20 failures that answer-only
...
\midrule
Refund trace suite & 10 & 100.0\% & 50.0\% \\
Tool-safety trace suite & 15 & 100.0\% & 0.0\% \\
External protocol trace suite & 5 & 100.0\% & 100.0\% \\
\midrule
Total & 30 & 100.0\% & 33.3\% \\
\bottomrule
\end{tabular}
\caption{Offline benchmark results generated by \texttt{uv run anvil bench experiments/paper.yaml --offline}.}
\label{tab:benchmark}
\end{table}

Across 30 trials, the final-answer baseline marks every trial as passing.
Agent Anvil's trace-aware checks expose 20 answer-only missed failures, all
...
\texttt{docs/paper/results.md}, and local trace artifacts under
\texttt{runs/paper-benchmark/}. The generated benchmark summary reports 30
total trials, 100.0\% final-answer pass rate, 33.3\% trace-aware pass rate, and
20 answer-only missed failures.
