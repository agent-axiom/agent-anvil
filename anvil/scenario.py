...
class AssertionCheck(BaseModel):
    ...
    def validate_required_fields(self) -> AssertionCheck:
        ...
        if self.path is not None and not _is_supported_json_path(self.path):
            raise ValueError(
                f"unsupported JSONPath {self.path!r}; use $, dot keys, and zero-based indexes"
            )
        return self


def _is_supported_json_path(path: str) -> bool:
    if path == "$":
        return True
    ...
    while remainder:
        if remainder.startswith("."):
            key, remainder = _consume_supported_json_path_key(remainder[1:])
            if key is None:
                return False
            continue
        if remainder.startswith("["):
            index, remainder = _consume_supported_json_path_index(remainder)
            if index is None:
                return False
            continue
        return False
...
def _consume_supported_json_path_key(remainder: str) -> tuple[str | None, str]:
...
def _consume_supported_json_path_index(remainder: str) -> tuple[int | None, str]:
