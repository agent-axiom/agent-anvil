...
- [x] Write failing tests for successful verification, repository/source policy command arguments, subject SHA-256 binding, JSON output, and report persistence.
- [x] Run `uv run pytest tests/test_attestations.py -q` and confirm failures are caused by the missing module and command.
- [x] Implement `LeaderboardArtifactAttestationVerification` and `verify_leaderboard_artifact_attestation` with a replaceable command runner.
- [x] Add `leaderboard verify-attestation` with controlled text/JSON output and policy options.
...
- [x] Write failing tests for missing `gh`, timeout, non-zero exit, malformed JSON, empty verification results, and subject-digest mismatch.
- [x] Run the focused tests and confirm each new test fails for the intended reason.
- [x] Implement bounded controlled errors without persisting raw bundles or unbounded stderr.
- [x] Run the focused tests and confirm they pass.
...
- [x] Write failing contract registry and golden-fixture tests.
- [x] Register the Pydantic model and export the schema with `uv run anvil schema export --out schemas`.
- [x] Add the valid fixture and document the public contract.
- [x] Run `uv run pytest tests/test_contracts.py -q` and confirm it passes.
...
- [x] Write failing tests showing batch verification links an attestation report and audit accepts it without running GitHub CLI.
- [x] Write failing tests showing `--require-artifact-attestation` rejects missing, malformed, or digest-mismatched reports.
- [x] Add optional attestation fields to evidence refs and audit rows, preserving old v1 documents through defaults.
- [x] Add `verify-all --artifact-attestations` and `audit --require-artifact-attestation`.
- [x] Re-export schemas and run focused leaderboard tests.
...
- [x] Update the copy-paste workflow to generate attestation evidence before enforcing it in audit.
- [x] Explain online and bundle-based verification, signer narrowing, self-hosted policy, and the distinction between GitHub-run and artifact provenance.
- [x] Add documentation assertions for the new command and workflow flags.
